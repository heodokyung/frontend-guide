...
> jQuery로 익숙하게 처리하던 DOM, 이벤트, 스타일, Ajax 코드를 순수 JavaScript로 옮길 때 참고하는 문서입니다.
...
그래서 신규 프론트엔드 프로젝트에서는 가능한 한 순수 JavaScript를 기본으로 두고, 꼭 필요한 경우에만 외부 라이브러리를 더하는 편이 좋습니다.

예전에 정리해둔 jQuery → 순수 JavaScript 변환 메모를 바탕으로, 작업 중에 바로 찾아볼 수 있게 다시 정리했습니다.
...
이 문서는 제가 예전에 정리한 jQuery → 순수 JavaScript 변환 메모를 바탕으로, 현재 프론트엔드 작업 기준에 맞게 다시 정리했습니다.
