...
> 허도경이 실제 프론트엔드 작업에서 기준으로 삼기 위해 정리한 HTML/CSS 컨벤션입니다.

마크업은 화면의 뼈대이고, CSS는 그 뼈대에 질서를 부여하는 방식입니다. 이 문서는 예쁜 화면을 만들기 전에, 누가 봐도 구조를 이해할 수 있고 오래 고칠 수 있는 코드를 만드는 데 초점을 둡니다.
...
좋은 마크업과 CSS는 화려한 코드가 아닙니다. 같은 화면을 다른 사람이 맡아도 비슷한 구조로 이어갈 수 있는 코드입니다.
